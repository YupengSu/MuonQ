...
from .utils import get_worker_info, save_checkpoint, load_checkpoint
import json
...
def nested_tensor_bytes_dedup(obj, seen=None) -> int:
    if seen is None:
        seen = set()
    ...
    if torch.is_tensor(obj):
        ...
        for v in obj.values():
            total += nested_tensor_bytes_dedup(v, seen)
...
def muon_state_bytes(optimizer) -> int:
    seen = set()
    total = 0
    ...
    for p, state in optimizer.state.items():
        ...
        total += nested_tensor_bytes_dedup(state, seen)
    ...
    for epoch in range(training_params['num_epochs']):
        ...
        for batch in train_dataloader:            
            ...
            if step % grad_accum_steps != 0:
                ...
                muon_state_mb = muon_state_bytes(optimizer) / (1024 ** 2)
                logger.memory_allocated.append(round(mem_allocated_mb, 2))
                logger.muon_state_mb.append(round(muon_state_mb, 2))
                ...
                if master_process and wandb_run is not None:
                    wandb_log_dict = {
                        ...
                        "train/step": step,
                        "train/memory_allocated_mb": mem_allocated_mb,
                        "train/muon_state_mb": muon_state_mb,
                    }
                ...
                if (step % logging_params['log_step'] == 0) & master_process:
                    ...
                    print(f"Memory : allocated={mem_allocated_mb:.1f}MB | muon_state={muon_state_mb:.1f}MB")
